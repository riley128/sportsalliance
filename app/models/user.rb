  ...
  attr_accessible :email, :hometown, :password, :password_confirmation, :remember_me
	 ...
	 def self.from_omniauth(auth)
	  where(auth.slice(:provider, :uid)).first_or_initialize.tap do |user|
	    ...
	    user.urls = auth.info.urls.Facebook if auth.info.urls.Facebook
	    user.location = auth.info.location if auth.info.location
	    user.verified = auth.info.verified if auth.info.verified
	    user.expires = auth.credentials.expires if auth.credentials.expires
	    user.gender = auth.extra.raw_info.gender if auth.extra.raw_info.gender
	    user.timezone = auth.extra.raw_info.timezone if auth.extra.raw_info.timezone
	    user.locale = auth.extra.raw_info.locale if auth.extra.raw_info.locale
	    user.hometown = auth.extra.raw_info.hometown.name if auth.extra.raw_info.hometown.name
	  end
	end
